...
/// Tela principal do app (RF06 / RQ01).
///
/// Exibe dois painéis:
///   • Lista horizontal "Urgente" com pets perdidos (RF07)
///   • Feed com abas Perdidos / Resgatados (RF06 / RF08)
...
class _PetSaveHomePageState extends State<PetSaveHomePage> {
  ...
  void _refreshLists() => setState(() => _refreshToken++);
          ...
          child: Column(
            ...
            children: [
              ...
              Padding(
                ...
                child:
                    _RecentFeedList(key: ValueKey('recent_$_refreshToken')),
              ),
...
class _TopBar extends StatelessWidget {
  ...
  Future<void> _logout(BuildContext context) async {
    ...
    // Encerra a sessão Supabase Auth (RQ03)
                  ...
                  children: [
                    Container(
                      ...
                      decoration: const BoxDecoration(
                          color: _orangeSoft, shape: BoxShape.circle),
                      child:
...
class _UrgentPetsListState extends State<_UrgentPetsList> {
  late Future<List<PetCard>> _future;
  final _controller = HomeController();

  @override
  void initState() {
    super.initState();
    _future = _controller.getUrgentPets();
  }
...
class _PetCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final badgeLabel = pet.isResgatado ? 'RESGATADO' : 'PERDIDO';
...
class _RecentFeedListState extends State<_RecentFeedList> {
  ...
  void initState() {
    super.initState();
    _future = _controller.getAllPets();
  }
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<List<PetCard>>(
      ...
      builder: (context, snapshot) {
        ...
        final perdidos   = all.where((p) => !p.isResgatado).toList();
        final resgatados = all.where((p) => p.isResgatado).toList();
        final list       = _selectedIndex == 0 ? perdidos : resgatados;
          ...
          children: [
            ...
            Container(
              ...
              child: Row(
                children: [
                  _TabButton(
                    label: 'Perdidos (${perdidos.length})',
                    selected: _selectedIndex == 0,
                    selectedColor: const Color(0xFFEF4444),
                    onTap: () => setState(() => _selectedIndex = 0),
                  ),
                  _TabButton(
                    label: 'Resgatados (${resgatados.length})',
        ...
        child: Container(
          ...
          child: Center(
            child: Text(
              label,
              style: TextStyle(
                color: selected ? selectedColor : _textSecondary,
                fontWeight:
        ...
        children: [
          Container(
            decoration: BoxDecoration(
              ...
              border:
                  Border.all(color: _divider.withOpacity(0.5)),
            ),
            child: ClipRRect(
              borderRadius: BorderRadius.circular(12),
              child: Image(
